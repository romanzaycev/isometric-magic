...
""");
    ...
    {
        var fileName = $"ion_motion_tests_{Guid.NewGuid():N}.ini";
        var basePath = Path.Combine(AppContext.BaseDirectory, fileName);
        var tempDir = Path.Combine(Path.GetTempPath(), $"ion_motion_tests_cwd_{Guid.NewGuid():N}");
...
""");
    ...
    {
        var missingFileName = $"ion_motion_missing_{Guid.NewGuid():N}.ini";
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"ion_motion_tests_{Guid.NewGuid():N}.ini");
